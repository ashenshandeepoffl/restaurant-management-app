    ...
    <section id="special-offers" className="py-16 bg-gray-50">
      <div className="container mx-auto px-4 text-center max-w-6xl">
        <div className="mb-12">
          ...
          <h3 className="text-5xl font-bold text-gray-800 mt-2">Exclusive Deals</h3>
        ...
        {loading ? (
          ...
          <Slider {...settings}>
            {promotions.map((promo) => (
              <div key={promo.promo_id} className="px-2">
                <div className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-2xl transition-shadow transform hover:scale-105 duration-300 flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <h4 className="text-2xl font-bold text-gray-900 mb-2">{promo.title}</h4>
                    <p className="text-gray-700 mb-4 flex-grow">{promo.description}</p>
                    <p className="text-green-500 font-semibold">
                      {promo.discount_percentage}% Off
                    </p>
                    <p className="text-gray-600 mt-2">
                      <strong>Start Date:</strong> {new Date(promo.start_date).toDateString()}
                    </p>
                    <p className="text-gray-600">
                      <strong>End Date:</strong> {new Date(promo.end_date).toDateString()}
                    </p>
                  </div>
                </div>
              </div>
